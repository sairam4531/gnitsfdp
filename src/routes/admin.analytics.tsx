...
function AnalyticsPage() {
  ...
  const daily = Array.from({ length: 30 }).map((_, i) => {
    ...
    return { date: format(d, "MMM d"), count: day.length, revenue: day.reduce((s, r) => s + (r.registration_fee || 0), 0) };
  });
  ...
  const dept = Object.entries(regs.reduce<Record<string, number>>((a, r) => {
    const k = r.department === "Others" ? r.custom_department || "Others" : r.department;
    a[k] = (a[k] || 0) + 1; return a;
  }, {})).map(([name, value]) => ({ name, value }));

  const inst = Object.entries(regs.reduce<Record<string, number>>((a, r) => {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-2xl font-bold">Analytics</h1><p className="text-sm text-muted-foreground">Detailed registration analytics.</p></div>
      <div className="grid gap-6 lg:grid-cols-2">
        <ChartCard title="Daily Registrations (30 days)">
          <BarChart data={daily}>
            <CartesianGrid strokeDasharray="3 3" opacity={0.3} /><XAxis dataKey="date" fontSize={10} /><YAxis fontSize={10} allowDecimals={false} /><Tooltip />
            <Bar dataKey="count" fill="#7c3aed" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ChartCard>
        <ChartCard title="Weekly Trend (8 weeks)">
          <LineChart data={weekly}>
            <CartesianGrid strokeDasharray="3 3" opacity={0.3} /><XAxis dataKey="week" fontSize={10} /><YAxis fontSize={10} allowDecimals={false} /><Tooltip />
            <Line type="monotone" dataKey="count" stroke="#a855f7" strokeWidth={2.5} />
          </LineChart>
        </ChartCard>
        <ChartCard title="Revenue Trend (30 days)">
          <LineChart data={daily}>
            <CartesianGrid strokeDasharray="3 3" opacity={0.3} /><XAxis dataKey="date" fontSize={10} /><YAxis fontSize={10} /><Tooltip />
        ...
        <ChartCard title="Department-wise">
          <BarChart data={dept}>
            <CartesianGrid strokeDasharray="3 3" opacity={0.3} /><XAxis dataKey="name" fontSize={10} angle={-20} textAnchor="end" height={60} /><YAxis fontSize={10} allowDecimals={false} /><Tooltip />
            <Bar dataKey="value" fill="#7c3aed" radius={[4, 4, 0, 0]} />
          </BarChart>
        </ChartCard>
        <ChartCard title="Institute-wise">
          <BarChart data={inst} layout="vertical">
            <CartesianGrid strokeDasharray="3 3" opacity={0.3} /><XAxis type="number" fontSize={10} allowDecimals={false} /><YAxis dataKey="name" type="category" fontSize={10} width={120} /><Tooltip />
