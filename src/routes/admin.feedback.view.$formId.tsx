...
const COLORS = ["hsl(var(--primary))", "hsl(var(--secondary))", "hsl(var(--accent))", "#f59e0b", "#ef4444", "#10b981", "#6366f1"];
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <Button asChild variant="ghost" size="sm">
          <Link to="/admin/feedback/responses"><ArrowLeft className="mr-1 h-4 w-4" /> Back</Link>
        </Button>
      </div>
      ...
      <Card>
        <CardContent className="p-6">
          ...
          <div className="mt-2 flex flex-wrap gap-x-6 gap-y-1 text-sm text-muted-foreground">
            <span>Button: <strong>{form?.feedback_button_name}</strong></span>
            {form?.feedback_date && <span>Date: {form.feedback_date}</span>}
            <span>Total Responses: <strong>{responses.length}</strong></span>
          </div>
        </CardContent>
      </Card>
          ...
          if (q.question_type === "multiple_choice") {
            const counts = (q.options_json ?? []).map((opt) => ({
              ...
              value: responses.reduce(
                (acc, r) => acc + ((r.answers_json ?? []).some((a) => a.question_id === q.id && a.answer === opt) ? 1 : 0),
                0
              ),
            }));
            return (
              <Card key={q.id}>
                <CardHeader>
                  <CardTitle className="text-base">Q{qi + 1}. {q.question_text}</CardTitle>
                </CardHeader>
                <CardContent className="grid gap-6 lg:grid-cols-3">
                  <div>
                    <Table>
                      <TableHeader>
                        <TableRow><TableHead>Option</TableHead><TableHead className="text-right">Responses</TableHead></TableRow>
                      </TableHeader>
                      <TableBody>
                        {counts.map((c) => (
                          <TableRow key={c.name}>
...
function ShortAnswerCard({ qi, q, responses }: any) {
  ...
  const answers = responses
    ...
    .filter(Boolean)
    .filter((a: any) => !search || a.answer.toLowerCase().includes(search.toLowerCase()) || a.name.toLowerCase().includes(search.toLowerCase()));
    ...
    <Card>
      ...
      <CardContent>
        <div className="relative mb-3">
          ...
          <Input className="pl-8" placeholder="Search answers…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
        <Table>
          <TableHeader>
            <TableRow><TableHead>Participant</TableHead><TableHead>Answer</TableHead></TableRow>
          </TableHeader>
          <TableBody>
            {answers.length === 0 ? (
              <TableRow><TableCell colSpan={2} className="text-center text-muted-foreground">No answers.</TableCell></TableRow>
            ) : (
              answers.map((a: any, i: number) => (
                <TableRow key={i}>
                  <TableCell>{a.name}<div className="text-xs text-muted-foreground">{a.email}</div></TableCell>
