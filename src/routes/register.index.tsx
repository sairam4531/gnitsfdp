...
export const Route = createFileRoute("/register/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Register for the One Week FDP on Smart Data Visualization at GNITS." },
    ],
  }),
...
const schema = z.object({
  ...
  message: "Department name required", path: ["custom_department"],
}).refine(d => d.institute !== "Others" || (d.custom_institute && d.custom_institute.length > 0), {
  message: "Institute name required", path: ["custom_institute"],
});
    ...
    return (
      <div className="min-h-screen bg-background">
        ...
        <div className="container mx-auto max-w-2xl px-4 py-24 text-center">
          ...
          <p className="mt-3 text-muted-foreground">Registrations for this FDP are currently closed. Please check back later.</p>
        ...
        <form onSubmit={form.handleSubmit(onSubmit, onError)} className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <Field label="Department" error={form.formState.errors.department?.message}>
                <Select onValueChange={(v) => form.setValue("department", v as FormVals["department"], { shouldValidate: true })}>
                  <SelectTrigger><SelectValue placeholder="Select department" /></SelectTrigger>
                  <SelectContent>
                    {["CSE", "CSE (AI & ML)", "CSE (Data Science)", "IT", "ECE", "EEE", "Others"].map(d => <SelectItem key={d} value={d}>{d}</SelectItem>)}
                  </SelectContent>
                </Select>
              </Field>
              {department === "Others" && (
                <Field label="Department Name" error={form.formState.errors.custom_department?.message}>
                  <Input {...form.register("custom_department")} placeholder="Enter department name" />
                </Field>
              )}
              <Field label="Institute / Organization" error={form.formState.errors.institute?.message}>
                <Select onValueChange={(v) => form.setValue("institute", v as FormVals["institute"], { shouldValidate: true })}>
              ...
              {institute === "Others" && (
                <Field label="Institute Name" error={form.formState.errors.custom_institute?.message}>
                  <Input {...form.register("custom_institute")} placeholder="Enter institute name" />
                </Field>
              )}
          ...
          {institute && (
            <Card className="border-secondary/40 bg-gradient-to-br from-accent to-background shadow-elegant">
              <CardContent className="flex flex-wrap items-center justify-between gap-4 p-6">
                <div>
                  <div className="text-xs uppercase tracking-wider text-muted-foreground">Registration Category</div>
                ...
                <div className="text-right">
                  ...
                  <div className="flex items-center text-3xl font-black text-secondary">
                    <IndianRupee className="h-6 w-6" />{fee}
                  </div>
                </div>
              </CardContent>
            </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Scan & pay using the QR or UPI ID below, then enter your UTR and upload screenshot.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-3">
                <div className="flex items-center justify-center rounded-lg border bg-muted/30 p-4">
                  {payment?.qr_code_url ? (
                    ...
                    <div className="text-center text-muted-foreground"><QrCode className="mx-auto h-10 w-10" /><div className="mt-2 text-xs">QR not configured</div></div>
                  )}
                </div>
              ...
              <Field label="UTR / Transaction Number" error={form.formState.errors.utr_number?.message}>
              ...
              <div>
                ...
                <div className="mt-1 rounded-lg border border-dashed p-4 text-center">
                  ...
                  <p className="mt-1 text-xs text-muted-foreground">JPG, JPEG, PNG, PDF · max 10 MB</p>
                </div>
              </div>

              <label className="flex items-start gap-2 text-sm">
                <Checkbox checked={!!form.watch("declaration")} onCheckedChange={(v) => form.setValue("declaration", v ? true : (undefined as unknown as true), { shouldValidate: true })} />
                <span>I hereby declare that all information provided is correct.</span>
              </label>
              {form.formState.errors.declaration && <p className="text-sm text-destructive">{form.formState.errors.declaration.message}</p>}
            </CardContent>
          </Card>

          <Button type="submit" size="lg" disabled={submitting} className="w-full bg-gradient-primary text-primary-foreground font-bold shadow-elegant">
